...
export const updateChats = async (chatId, updatedData) => {
  try {
    const { data, error } = await supabase
      .from("chats")
      .update(updatedData) // NOT an array
      .eq("id", chatId) // specify row to update
      .select()
      .single();
    ...
    console.error("updateChats error:", err.message);
...
// Add this function to your supabase services file
...
export const fetchNotificationsLength = async (userId) => {
  try {
    ...
    console.error("fetchNotificationsLength error:", err.message);
    return { success: false, error: err.message };
  }
};
